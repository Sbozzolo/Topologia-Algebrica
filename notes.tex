...
\begin{lemma}
  Sia $ X $ un CW complesso finito i cui $ k $-scheletri sono $ X^{(0)}, \dots, X^{(k)} $, allora
  vale che:
  \begin{enumerate}
  \item
    \[
      H_k(X^{(n)}, X^{(n-1)}) \cong
      \begin{cases}
        \Z^{a_n} & \text{se } k = n \text{ con $ a_n $ numero di $ k $-celle} \\
        0 & \text{se } k \not = n
      \end{cases}
    \]
  \item
...
\begin{proof}
  ...
  \]
  Usandolo ho $ H_k(X^{(n)}, X^{(n-1)}) \cong \tilde{H}_k(\quot{X^{(n)}}{X^{(n-1)}}) $, ma
  \[
  ...
  Se $ k > n $ è nullo, se $ k < n $ è $ \Z $ per ogni cella.
  Questo è vero per induzione su $ t $.
  \[
    \tilde{h}_k(\Sph{n}) \cong
    \begin{cases}
      \Z     & \text{se } k = n \\
    ...
    \end{cases}
  \]
  Suppongo di conoscere $ \tilde{H}_k(\Sph{n}_1 \vee \dots \Sph{n}_t) $ voglio calcolare $ \tilde{H}_k(\Sph{n}_1 \vee \dots \Sph{n}_t) $.
  Come notazione pongo $ Z_t = \Sph{n}_1 \vee \dots \Sph{n}_t $ e $ B = \Sph{n}_t $, cioè $ Z_t = Z_{t-1} \vee B $
  vale la successione esatta lunga in omologia relativa:
  \[
    \begin{tikzcd}
      \dots \rar & H_k(Z_{t-1}) \rar & H_k(Z_t) \rar          & H_k(Z_t, Z_{t-1}) \rar & H_{k-1}(Z_{t-1}) \rar & \dots
    \end{tikzcd}
  \]
  Se $ k \not = n $ allora $ H_k(Z_{t-1}) = H_{t-1}(Z_{t-1}) = 0 $ quindi la successione è:
  \[
    \begin{tikzcd}
      0 \rar   & H_k(Z_t) \rar     & H_k(Z_t, Z_{t-1}) \rar & 0
    \end{tikzcd}
  \]
  Per $ k = 0 $ lo so calcolare, mi metto nel caso $ k \geq 2 $ e quindi
  $ H_k(Z_t) = 0 $ per ipotesi induttiva quindi $ H_k(Z_t) \cong H_k(Z_t, Z_{t-1}) $.
  Ma vale il risultato $ H_k(Y,A) \cong \tilde{H}_k(\quot{Y}{A}) $ quindi:
  \[
    \begin{tikzcd}
      H_k(Z_t, Z_{t-1}) \cong \tilde{H}_k(\quot{Z_t}{Z_{t-1}}) \cong \tilde{H}_k(\Sph{n}_t) \cong \tilde{H}_k(B)
    \end{tikzcd}
  \]
  Se invece $ k = n $ allora vale la successione esatta:
  \[
    \begin{tikzcd}
      \dots \rar \tilde{H}_{n+1}(\quot{Z_t}{Z_{t-1}}) \rar & H_n(Z_{t-1}) \rar & H_n(Z_t) \rar  & H_n(Z_t, Z_{t-1}) \rar & H_{n-1}(Z_{t-1}) \rar & \dots
    \end{tikzcd}
  \]
  Ma $ \quot{Z_t}{Z_{t-1}} \cong \Sph{n} $ quindi $ \tilde{H}_{n+1} \cong 0 $ e $ H_{n-1}(Z_{t-1}) \cong 0 $ per ipotesi induttiva
  quindi:
  \[
    \begin{tikzcd}
    ...
    \end{tikzcd}
  \]
  Quindi siccome la successione è spezzante $ H_n(Z_t) \cong \Z^t $. Questo era il primo punto.
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Ma adesso so quelli dell'omologia relativa, quindi se $ k \not \in \set{n, n-1} $ allora
  $ H_{k+1}(X^{(n)}, X^{(n-1)}) $ e $ H_k(X^{(n)}, X^{(n-1)}) $ sono nulli quindi la successione diventa:
  \[
    \begin{tikzcd}
      0 \rar   & H_k(X^{(n-1)}) \rar  & H_k(X^{(n)}) \rar & 0
    \end{tikzcd}
  \]
  Quindi $ H_k(X^{(n-1)}) \cong X_k(X^{(n)}) $. Per $ k \not \in \set{n, n-1} $  se $ k \geq \not = 0 $ allora
  $ H_k(X^{(0)}) = 0 $ in quanto $ X^{(0)} $ sono punti.
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Se $ k < n $ sicuramente $ k + 1 \not = n + 1 $ quindi $ H_{k+1}(X^{(n+1)}, X^{(n)}) \cong 0 $, ma anche $ H_k(X^{(n+1)}, X^{(n)}) $
  quindi $ H_k(X^{(n)}) \cong H_k(X^{(n+1)}) $.
  Quindi $ H_k(X^{(n)}) \cong H_k(X^{(n-1)}) \dots \cong H_k(X^{(n)}) \cong H_k(X) $.
\end{proof}

Dimostro il risultato che ho lasciato in sospeso:
\begin{lemma}
  Sia $ (Y,A) $ e $ q \colon (Y,A) \to \left(\quot{Y}{A}, \quot{A}{A} \right) $
  se $ Y = X^{(n)} $ e $ A = X^{(n-1)} $ allora esiste un intorno $ V $
  di $ A $ in $ X $ tale che $ A $ sia un retratto di deformazione di $ A $.
  Le coppie $ (Y,A) $ con $ A \subseteq Y $ entrambi dotati di struttura di CW complessi
  che soddisfano questa condizione sono dette \textbf{coppie buone}.
\end{lemma}
\begin{proof}
  Per la funtorialità:
  \[
    \begin{tikzcd}
      H_n(Y,A) \rar \dar{q_\star} & H_n(Y,V) \dar{q_\star} \\
  ...
  $ (Y,A) \incl (Y,V) $. A questa inclusione corrisponde la successione esatta lunga:
    \[
    \begin{tikzcd}
      \dots \rar & H_{n+1}(Y, A) \rar & H_n(Y, A) \rar & H_k(Y,V) \rar & H_{n-1}(Y,A) \rar & \dots
    \end{tikzcd}
  \]
  Ma $ V $è omotopa ad $ A $ quindi $ H_k(V,A) \cong H_k(A,A) \cong 0 $, infatti:
  \[
    \begin{tikzcd}
      0 \rar   & H_k(A) \rar  & H_k(A) \rar  & H_k(A,A) \rar & 0
    \end{tikzcd}
  \]
  Quindi $ H_k(Y,A) \cong H_n(Y,V) $. Per gli stessi motivi sono isomorfi
  $ H_n(\quot{Y}{A}, \quot{A}{A}) $ siccome $ V $ ha lo stesso tipo di omotopia
  di $ A $. Quindi ho:
  \begin{align*}
    i \colon A & \to V \\
    r \colon V & \to A
  ...
  \]
  è la retrazione cioè tale che $ \rho \circ j = \Id{\quot{A}{A}} $ e $ j \circ \rho = \Id{\quot{V}{A}} $.
  Questi si ottengono componendo l'omotopia con la proiezione al quoziente.
  Io ho $ A \subseteq V \subseteq Y $, faccio l'escissione di $ A $:
  \[
    \begin{tikzcd}
      H_n(Y,A) \rar{\cong} \dar{q_\star} & H_n(Y,V)  \dar{q_\star}& H_n(\quot{Y}{A}, \quot{V}{A}) \lar{\cong} \\
  ...
  Mi rimane da mostrare che $ H_k(\quot{Y}{A}, \quot{V}{A}) \cong H_n(\quot{Y}{A} \setminus \quot{A}{A}, \quot{V}{A} \setminus \quot{A}{A}) $.
  Ma questo è vero in quanto.
  In questo è fondamentale prendere l'aperto, ma nel CW complesso posso sempre farlo.
\end{proof}
\begin{exercise}
...
\]
Definisco $ S_k^{CW}(X) := H_k(X^{(k)}, X^{(k-1)}) $. So che $ (X^{(k+1)}, X^{(k-1)}) $ è una coppia
e ho la successione esatta in omologia:
\[
  \begin{tikzcd}
    \dots \rar H_{k+1}(X^{(k)}) \rar & H_{k+1}(X^{(k+1)}) \rar                                        & H_{k+1}(X^{(k+1)}, X^{(k)}) \rar           & H_k(X^{k})                   & \dots
  \end{tikzcd}
\]
...
Nel complesso ho:
\[
  \begin{tikzcd}[nodes={column sep=4pt}]
  0            \arrow{dr}{}      & {}                   & {}                                         & {}                           & 0  & {}\\
  {}                             & H_k(X^{(k)})   \arrow{dr}{}                 & {}                                    & H_k(X^{(k+1)})  \arrow{ur}{} & {} & {}\\
  {}                             & {}             & H_k(X^{(k)})   \arrow{dr}{j_k}   \arrow{ur}{\tau} & {}                           & {} & {}\\
  {}  & H_k(X^{(k+1)}, X^{(k)})  \arrow{ur}{\delta_{k+1}} \arrow{rr}{d_{k+1}^{CW}} & {}      & H_k(X^{(k)}, X^{(k-1)})  \arrow{rr}{d_{k}^{CW}}   \arrow{dr}{\delta_k}  & {} &  H_{k-1}(X^{(k-1)}, X^{(k-2)}) \\
  {} & {} & {} & {} & H_{k-1}(X^{k-1}) \arrow{ur}{j_{k-1}} & {} & {}
\end{tikzcd}
\]
...
\[
   d_k^{CW} \circ  d_k^{CW} = j_{k-1} \circ \delta_k \circ j_k \circ \delta_{k-1} = 0
\]
...
che:
\[
  H_k(X) \cong j_k\left(\quot{\ker{d_k^{CW}}}{\im{d_{k+1}^{CW}}}\right)
\]
Ho visto che se $ k < n $ allora $ H_k(X^{(n)}) \cong H_k(X) $ posso prendere $ n = k + 1 $
e ho $ H_k(X) \cong H_k(X^{(k+1)}) \cong \quot{H_k(X^{(k)})}{\ker{\tau}} $ in quanto la successione
è esatta. Ma $ \ker{\tau} = \im{\delta_{k+1}} $ quindi $ H_k(X) \cong \quot{H_k(X^{(k)})}{\im{\delta_{k+1}}} $.
Poi ho $ j_k \colon H_k(X^{(k)}) \to S_k^{CW}(X) $ cosa è $ j_k(\im{\delta_{k+1}}) $
\[
  j_k\left(\im{\delta_{k+1}}\right) = \im{j_k \circ \delta_{k+1}}
\]
Infatti se $ z \in j_k\left(\im{\delta_{k+1}}\right) $ allora $ z = j_k \circ \delta_{k+1}(u) $,
se $ w \in \im{j_k \circ \delta_{k+1}} $ allora $ w = j_k \delta_{k+1}(r) $. Quindi
$ j_k\left(\im{\delta_{k+1}}\right) = \im{d_{k+1}^{CW}} $, e perciò;
\[
  H_k(X) \cong \quot{H_k(X^{(k)})}{\im{d^{CW}_{k+1}}}
\]
Se faccio vedere che $ \im{d_{k+1}^{CW}} = \ker{d_k^{CW}} $ ho finito. Avevo mostrato che
$ H_k(X^{(k-1))}) = 0 $ quindi se $ n = k - 1 $ allora $ H_k(X^{(k-1)}) = 0 $ quindi
$ j_k $ è iniettiva. Devo capire cosa è $  j_k\left(\im{H_k(X^{(k)})}\right) $, questo è
$ \im{j_k} $ ma la successione è esatta quindi $ \im{j_k} = \ker{\delta_k} $.

Se dimostro che $ \ker{\delta_k} \cong \ker{d_k^{CW}} $ ho finito.
Quindi $ j_k(H_k(X^{(k)})) \cong \ker{\delta_k} \cong \ker{d_k^{CW}} $ quindi
\[
  H_k(X) \cong \quot{ker{d_k^{CW}}}{d_{k+1}^{CW}} \cong H_k^{CW}(X)
\]
Ma quello che ho detto prima è vero perché se $ z \in \ker{\delta_k} $:
\[
  \delta_k(z) = 0 \iff j_{k-1} \circ \delta_k(z) = 0 \Rightarrow z \in \ker{j_k \circ \delta_k} = \ker{d_k^{CW}}
\]
Ho $ j_{k-1} \colon \ker{\delta_k} \to \ker{d_k^{CW}} $ e $ j_{k-1} \left(\ker{\delta_k} \right) \subseteq \ker{d_{k}^{CW}} $.
Mostro l'inclusione inversa:
Se $ w \in \ker{d_k^{CW}} $ allora $ 0 = d_k^{CW}(w) = j_{k-1} \circ \delta_k (w) $.
Quindi $  j_{k-1} \circ \delta_k (w) = 0 $ ma $ j_{k-1} $ è iniettivo e perciò:
$ \delta_k(w) = 0 $ quindi $ w \in \ker{\delta_k} $.
