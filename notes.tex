S\documentclass[10pt, twoside=false, x11names]{scrbook}
...
\[
  ...
  \end{tikzcd}
\]
I quadrati sono commutativi quindi questa succession esatta corta
...
Ho già visto che $ H_k(P) = H_k(P, \emptyset) = 0 $ per $ k \geq 1 $ e $ P $ insieme
formato da un solo punto, inoltre so che $ H_k (\invamalg_\alpha X_\alpha) \cong \bigoplus_\alpha H_k(X_\alpha) $

Quindi gli assiomi di Eilenberg e Steenrod non definiscono una teoria
vuota ma c'è almeno una teoria a soddisftarli.

In futuro verificherò gli assiomi mancanti.

Sia $ X \not = \emptyset $ spazio topologico e $ p \in X $ punto, allora ho le
...
Definisco (deriva dagli assioni ponendo $ k = 0 $) $ \epsilon_\star H_0(X) \to H_0(p) $,
$ \epsilon_\star $ è suriettiva perchè c'è $ i_\star \colon H_0(p) \to H_0(X) $:
\[
  \begin{tikzcd}
    0 \arrow{r}{} & \ker{\epsilon_\star} \arrow{r}{} & H_0(X) \arrow{r}{\epsilon_\star} & \arrow[bend right]{ll}{i_\star} H_0(p) \arrow{r}{} & 0
  \end{tikzcd}
\]
Inoltre per le proprietà funtoriali $ (\epsilon \circ i)_\star = (\Id{p})_\star = \Id{H_0(p)} $
e $ (\epsilon \circ i)_\star = \epsilon_\star \circ i_\star $ quindi $ \epsilon_\star \circ i_\star = \Id{H_0(p)} $.
La successione spezza quindi $ H_0(X) \cong \ker{\epsilon_\star} \oplus H_0(p) $.
Per qualsiasi teoria omologia che soddisfa gli assioni di Eilenberg
e Steenrod, e quindi in particolare per l'omologia singolare relativa,
si ha che $ H_0(X) \cong \ker{\epsilon_\star} \oplus H_0(p) $.

Generalmente si chiama $ H_0(p) $ il \textbf{gruppo dei coefficienti}\index{Gruppo dei coifficienti}
di una teoria omologica. Nell'omologia singolare relativa questo è $ \Z $.
Inoltre si definisce $ \ker{\epsilon_\star} = \tilde{H}_0(X) $ gruppo di omologia ridotta di ordine zero.

Cosa sono invece gli $ \tilde{H}_k(X) $? Vorrei che fossero i gruppi di omologia,
...
\end{proposition}
\begin{proof}
  Cibsuderi $ F \colon (X, A) \to (p,p) $ con:
  \[
    F =
    \begin{cases}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Per $ k \geq 1 $ $ \ker{\epsilon_X} = H_k(X) $, mentre per $ k = 0 $ $ H_0(X) \cong \tilde{H}_0(X) \oplus H_0(p) $,
  quindi:
  \[
    \tilde{H}_k(X) = \ker{\epsilon_C} =
    \begin{cases}
      H_k(X)               & \text{per } k \geq 1 \\
      \tilde{H}_0(X) \oplus H_0(p)
    \end{cases}
  \]
  Questo lo posso fare anche nel caso di una coppia.
  \[
    \begin{tikzcd}
      0 \arrow{r}{}        & \ker{F_X} \arrow{r}{} & H_k(X, A) \arrow{r}{} & H_k(p, p) \arrow{r}{} & 0
    \end{tikzcd}
  \]
  E si definisce $ \tilde{H}_k(X,A) = \ker{F_\star} $.
  Calcolo $ H_k(P,P) $ con $ P $ spazio formato da un solo punto in $ X $.
  So che c'è una successione esatta lunga:
  \[
    \begin{tikzcd}
      \dots \arrow{r}{}        & H_k(P) \arrow{r}{}    & H_k(P) \arrow{r}{}    & H_k(P,P) \arrow{r}{}  & H_{k-1}(P) \arrow{r}{} & \dots
    \end{tikzcd}
  \]
  La coppia è $ (P, P) = (X, A) $ quindi $ X = P $ e $ A = P $. Supponendo $ k \geq 2 $ la successione diventa:
  \[
    \begin{tikzcd}
      \dots \arrow{r}{}        & 0 \arrow{r}{}         & 0 \arrow{r}{}         & H_k(P,P) \arrow{r}{}  & 0 \arrow{r}{}          & \dots
    \end{tikzcd}
  \]
  Allora $ H_k(P, P) = 0 $. Mentre se $ k = 1 $ allora:
  \[
  ...
  \[
    \begin{tikzcd}[nodes={column sep=10pt}]
      \dots \arrow{r}{}   & 0 \arrow{r}{}         & 0 \arrow{r}{}         & H_1(P,P) \arrow{r}{}  & H_{0}(P) \arrow{r}{}   &
      H_{0}(P) \arrow{r}{} & H_{0}(P,P) \rar       & 0
    \end{tikzcd}
  \]
  Ma quindi:
  \[
    \begin{tikzcd}[nodes={column sep=10pt, row sep=10pt}]
      {} & {} & {} & {} & 0  \arrow{dr}{} & {} & {}  & 0 \arrow{dr}{} & {} & {}  & {}    \\
      \dots \rar & 0 \rar  & 0 \rar & H_1(P,P) \arrow{rr}{}  \arrow{ur}{}  & {}  & H_{0}(P) \rar &  H_{0}(P) \arrow{rr}{}  \arrow{ur}{}
      & {} & H_{0}(P,P) \rar & 0
    \end{tikzcd}
  \]
  E quindi $ H_1(P,P) = H_0(P,P) = 0 $.
\end{proof}
...
\begin{proof}
  ...
  $ f_\star \circ g_\star = \Id{H_k(P)} $ e  $ g_\star \circ f_\star = \Id{H_k(X)} $ con $ f_\star \colon H_k(X) \to H_k(P) $
  e  $ g_\star \colon H_k(P) \to H_k(X) $, quindi $ f_\star $ e $ g_\star $ sono inversi, cioè:
  $ (f \circ g)_\star = \left( \Id{P} \right)_\star $ e $ (g \circ f)_\star = \left( \Id{X} \right)_\star $
  per l'assiome dell'omotopia e per la funtorialità.
  Quindi:
  \[
    H_k(P) \cong H_k(X) \cong
    \begin{cases}
      \tilde{H}_k(X) = 0 & \text{se } k \geq 1 \\
      \tilde{H}_0(X) \oplus H_0(X) = H_0(X) & \text{se } k = 0
    \end{cases}
  \]
\end{proof}
