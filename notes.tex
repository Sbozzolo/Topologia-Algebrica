...
\subsection{Simplessi singolari}
...
\begin{proof}
  ...
  di base $ x_0 $: $ \lambda_{f(0)} \star f \star \bar{\lambda}_{f(1)} =: \tilde{f} $. Vale che $ \psi(f) = \llbracket \tilde{f} \rrbracket $.
  Bisogna mostrare che:
  \begin{enumerate}
  \item $ \psi $ è ben definito, cioè se $ f \sim g $ allora $ \psi(f) = \psi(g) $ e che $ \psi $
...
\[
  \phi \colon \quot{\pi_1(X)}{[ \pi_1(X), \pi_1(X)]} \homoto H_1(X)
\]
...
mi chiedo $ g $ induce un'applicazione tra $ H_k(X) $ e $ H_k(Y) $?
...
Si definisce $ g_\sharp \colon S_k(X) \to S_k(Y) $ tale che se $ c = \sum_\sigma n_\sigma \sigma $
allora $ c' = \sum n_\sigma (g \circ \sigma) $.
Questa mappa è ben definita ed è lineare: $ g_\sharp $ è un omomorfismo di
...
Definisco $ g_\star \colon H_k(X) \to H_k(Y) $.
Si dice che $ g $ è \textbf{covariante} perché va da $ X $ a $ Y $.
Considero un $ k $-ciclo $ c $, cioè tale che $ \partial c = 0 $ definisco
\[
  g_k([c]) = [g_\sharp (c)]
\]
Devo verificare se questa applicazione è ben definita:
considero $ d \in S_k(X) $ tale che $ \partial d = 0 $, suppongo
che $ d \sim c $, questo vale se e solo se $ [d] = [c] $,
mi chiedo è vero che $ g_\star([d]) = g_\star([c]) $?
Devo cioè mostrare che $ g_\sharp (d) \sim g_\sharp (c) $, ma questo è vero
...
ma $ d $ w $ c $ sono omologhi per ipotesi, quindi:
\[
  \exists u \in S_{k+1}(X) \; | \; \partial u = d - c
\]
Quindi $ g_\sharp(\partial u) = g_\sharp (d - c) $, e quindi:
$ [g_\sharp (d)] = [g_\sharp(c)] $ vorrei che questo sia implicato $ g_\sharp (\partial u) = g_\sharp (d - c) $.
Voglio trovare da $ u $ $ \tau $.
\begin{align*}
...
\begin{align*}
  ...
  \end{align*}
In particolare $ g_\star $ è omomorfismo in quanto è il passaggio a quoziente di omomorfismi.

Esempi:
\begin{itemize}
\item Sia $ j \colon \Sph{1} \to \Sph{2} $ che cosa è $ j_\star \colon H_1(\Sph{1}) \to H_1(\Sph{2}) $?
$ j_\star $ è una mappa costante in quanto $ \Sph{2} $ è contraibile, inoltre $ j $ era iniettiva,
ma $ j_\star $ è costante quindi non è più iniettiva.
\item Se considero $ \Sph{1} = \set{z \in \mathbb{C} | |z| = 1 } $
  \begin{align*}
  ...
  \begin{align*}
    ...
    [\sigma] & [f_\sharp (\sigma)] = [f \circ \sigma]
  \end{align*}
  Si ha:
  \[
    \begin{tikzcd}
      \Delta_1 \arrow{r}{\sigma} & \Sph{1} \arrow{r}{\partial f} & \Sph{1}
    \end{tikzcd}
  \]
  Con:
   \[
    \begin{tikzcd}
      t \arrow{r}{\sigma} & \me^{2 \pi i t} \arrow{r}{\partial f} & \me^{8 \pi i t}
    \end{tikzcd}
  \]
  Quindi:
  \begin{align*}
  ...
  \begin{align*}
    ...
    [\sigma] & \mapsto [\sigma^4]
  \end{align*}
  $ f_\star $ è iniettivo ma non suriettivo (con tutti gli interi sono
  multipli di 4)
\end{itemize}
...
\begin{osservation}
  ...
  $ g_\star \circ f_\star =  \left(g \circ f\right)_\star $, infatti:
  \[
    \left(g \circ f\right)_\star ([\sigma]) = [ (g \circ f) \circ \sigma ] =  [ g \circ (f \circ \sigma) ] =
    [ g_\sharp (f \circ \sigma)] = [ g_\sharp \circ f_\sharp (\sigma) ]
  \]
\end{osservation}
Quindi sulla categoria degli spazi topologici questo
fornisce un funtore covariante.
...
$ f_\sharp (S_\bullet(X), \partial) \to  (S_\bullet(Y), \partial') $ la condizione
di commutatività è $ f_\sharp \circ \partial = \partial' \circ f_\sharp $ che è
proprio quella che ho utilizzato poco fa e che
serve a preservare proprietà geometriche.
...
\begin{definition}
  ...
  $ \alpha $ omomorfismo iniettivo, $ \beta $ omomorfismo suriettivo
  e si richiede che $ \forall k $ sia $ C_k \cong \quot{B_k}{A_k} $.
\end{definition}
...
\[
  ...
  \end{tikzcd}
\]
Cioè per $ k $ fissato ho successioni esatte, quindi
l'immagine è uguale al nucleo e la mappa è iniettiva.
