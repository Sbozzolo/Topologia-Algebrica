...
\begin{definition}
  ...
  $ x_1, x_2, \dots $. In questo caso si indica
...
\begin{theorem}[Teorema di struttura per gruppi abeliani finitamente generati]
  ...
  elementi sono annullati elementi non nulli di $ \Z $.
\end{theorem}

\begin{definition}
  Un gruppo abeliano si dice \textbf{libero}\index{$ \Z $-modulo libero} se è la sua
...
\begin{definition}
  ...
  \]
  $ \quot{\M}{\sim} $ è l'insieme delle classi di equivalenza di $ \sim $ equipaggiate
...
\item $ \pi_3(\Sph{2}) \simeq \Z $\footnote[$\dagger$]{Questo da origine alla fibrazione di Hopf che ha molte applicazioni in fisica.}
\end{enumerate}
