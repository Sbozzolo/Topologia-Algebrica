...
\begin{definition}
  ...
  \]
  Dove $ F(A,B) $ è il gruppo libero generato da $ A \times B $ e $ R(A,B) $ il gruppo
  ...
  \begin{gather*}
    ...
    a \otimes (b_1 + b_2) = a \otimes b_1 + a \times b_2 \\
...
\end{proposition}
\begin{proof}
  La costruzione di $ \phi $ è banale, bisogna solo verificare che è ben definita. Sia
  $ \phi(a \otimes b) = \phi(\pi(a,b)) = \psi(a,b) $. Se $ \pi((c,d)) = a \otimes b $ allora
  $ (c,d) - (a,b) \in R(A,B) $, devo fare vedere che $ \psi((c,d)) = \psi((a,b)) = \phi(a \otimes b) $,
  cioè per linearità devo mostrare che $ \psi((c,d) - (a,b)) = 0 $, ma
  $ (a,b) $ e $ (c,d) $ sono in $ R(A,B) $, quindi:
  \[
    \psi((c,d) - (a,b)) = \sum_\alpha \psi((r_\alpha, s_\alpha)) = \sum_\alpha \phi(\pi((r_\alpha, s_\alpha))) = 0
  \]
  perché al quoziente vanno a zero.
\end{proof}
...
\begin{proposition}
  ...
  \end{align*}
  Allora $ f \otimes g $ è omomorfismi di gruppi abeliani.
\end{proposition}
\begin{proof}
  Ovvio.
\end{proof}
...
\end{proposition}
\begin{proof}
  Se per la proprietà universale trovo una mappa bilineare continua
...
\[
  ...
  \end{tikzcd}
\]
Mi chiedo: questa successione è esatta?
\begin{example}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Usando il lemma sotto $ 6x \otimes y = 6 xy $. Cerco il nucleo $ 6 xy = 0 $ quindi o $ x = 0 $ o $ y = 0 $,
  quindi sicuramente $ x \otimes y $ è zero e quindi $ \alpha \otimes \Id{} $ è inieittiva.
  L'applicazione:
  \begin{align*}
    \Z \otimes \Z & \quot{\Z}{6 \Z} \otimes \Z \\
    m \otimes n & \mapsto \bar{m} \otimes n
  \end{align*}
  Con $ \ bar{m} $ la classe modulo resto 6 di $ m $. Questa applicazione è
  suriettiva, infatti se prendo $ \bar{k} \otimes u $ con
  $ \bar{k} \in \quot{\Z}{6 \Z} $ e $ u \in \Z $, la preimmagine è
  $ k \otimes u $. Il nucleo è $ \bar{m} \otimes n = 0 $, cioè $ \bar{m} n = 0 $ in
  $ \quot{\Z}{6\Z} $ via isomorfismo. Se $ m = 6t $ allora $ \bar{m} = 0 $
  quindi $ \ker{\beta \otimes \Id{}} $ contiene $ \Z \otimes \Z $, cioè
  $ \Z \otimes \Z \subseteq \ker{\beta \otimes \Id{}} $. Ma $ \bar{m} \otimes n = n \bar{m} \otimes \Id{} = \bar{nm} \otimes \Id{} $,
  quindi $ mn = 6t $, ma $ n $ è intero, quindi $ m $ deve essere multiplo di 6, e quindi
  è vera anche l'inclusione inversa. La successione è quindi esatta.
\end{example}
\begin{example}
  Considero la stessa successione di prima, ma ora tensorizzo per $ \quot{\Z}{4 \Z} $,
  considero l'applicazione:
  \begin{align*}
    6 \Z \otimes \quot{\Z}{4 \Z} & \to \Z \otimes \quot{\Z}{4 \Z} \\
    6 x \otimes \bar{k} & \mapsto 6x \otimes \bar{k}
  \end{align*}
...
\begin{proof}
  ...
  \[
    \tau(a + b) \otimes \Id{} = a \otimes \Id{} + b \otimes \Id{} = \tau(a) + \tau(b)
  \]
  \begin{gather*}
    \sigma(\tilde{a} \otimes n + \tilde{b} \otimes n ) = \sigma(n \tilde{a} \otimes \Id{} + n \tilde{b} \otimes
    \Id{}) = \sigma (n \tilde{a} + n \tilde{b}) \otimes \Id{} = \\ = n \tilde{a} + n \tilde{b} =
    \sigma(\tilde{a} \otimes n) + \sigma(\tilde{b} \otimes n)
  \end{gather*}
  ...
  \[
    \begin{tikzcd}[nodes={row sep = 5 pt}]
      ...
      a \otimes n \arrow[mapsto]{r}{\sigma} & n \tilde{a} \arrow[mapsto]{r}{\tau} & n \tilde{a} \otimes \Id{} = \tilde{a} \otimes n
    \end{tikzcd}
  \]
\end{proof}
Da questi esempi capisco che in generale successioni esatte non vanno in successioni esatte,
...
Cioè:
\[
  [\alpha(r) \otimes g]' = [(\alpha(r) \otimes \Id{})(r \otimes g)]' = 0
\]
...
Ma a questo punto $ \quot{F}{\alpha(R)} \otimes G \cong A \otimes G $ quindi $ A \otimes G \cong \quot{F \otimes G}{{(\alpha \otimes \Id{})(R \otimes G)}} $.
...
\begin{definition}
  ...
  \[
    \begin{tikzcd}
      0 \rar & \ker{\alpha \otimes \Id{}} \rar & R \otimes G \rar{\alpha} & F \otimes G \rar{\beta} & A \otimes G \rar & 0
    \end{tikzcd}
  \]
...
\begin{definition}
  ...
  gruppo $ \ker{\alpha \otimes \Id{}} $, e lo si indica con $ \tor{A,G} $. Quindi vale che:
    \[
    \begin{tikzcd}
      0 \rar & \tor{A,G} \rar & R \otimes G \rar{\alpha} & F \otimes G \rar{\beta} & A \otimes G \rar & 0
    \end{tikzcd}
  \]
\end{definition}
\begin{lemma}
