...
\begin{definition}
  ...
  \item Il \textbf{$ k $-scheletro}\index{$ k $-scheletro} $ X^{(k)} $ si costruisce induttivamente
    ...
    \[
      X^{(k)} = X^{(k-1)} \cup_\phi \bigcup_\alpha \Disk{k}_\alpha = \bigcup_\alpha \quot{ X^{(k-1)} \sqcup \Disk{k}_\alpha}{x \sim \phi_\alpha(x)} \quad \forall \alpha \text{ e }
...
\begin{lemma}
  ...
  $ a $ è la mappa antipodale.
\end{lemma}

Si trova che:
\begin{itemize}
\item $ \Pjr{1} = \Sph{1} = \RN{} \cup {\infty} $
...
\begin{proof}
  ...
  \begin{gather*}
    ...
    (K \setminus D') \cap D' \sim \Sph{1} \; (\text{infatti è una corona circolare})
  \end{gather*}
  Inoltre $ D \sim \set{q} $ quindi $ K \setminus D' \sim K \setminus \set{q} $.
  [FIGURA]
  % So che
  % \[
