...
\begin{proof}
  ...
  \begin{gather*}
   ...
   \end{gather*}
   Considerando $ \vec{u} \otimes \vec{w} $ al posto di
   $ \vec{u} \otimes \vec{w} $ con $ \vec{u} \not = \vec{v} $ questo elemento ha una
   espansione su $ \set{g_k} $ ma $ \alpha $ è iniettiva quindi necessariamente anche
   $ \alpha({\vec{u}}) \otimes \vec{w} $ ha un'espansione diversa su
...
\begin{example}
  ...
  \item Per determinare $ d_2 $ uso la consueta formula: $ d_2 $ è la
    ...
    non fa nulla, e siccome c'è una sola cella la selezione della sfera è unica:
    \[
      \begin{tikzcd}
        \partial \Disk{2} = \Sph{1} \rar{f_n} \arrow{rd}{\phi} & X^{(1)} \dar{\pi}  & {} \\
        {} & \quot{X^{(1)}}{X^{(0)}} = X^{(1)} \rar{\eta} & \Sph{1}
      \end{tikzcd}
    \]
...
\begin{osservation}
  Assumendo $ G $ e $ H_k(X) $ finitamente generati per il teorema di struttura:
  \[
    G \cong \Z^{s_n} \oplus T \quad H_n(X) \cong \Z^{r_n} \oplus T' \quad H_{n-1}(X) \cong \Z^{t_n} \oplus T''
  \]
  % In generale se $ G $ è un gruppo abeliano finitamente generato c'è il teorema
  % di struttura per cui $ G \cong \Z^n \oplus T $,
  Per teorema dei coefficienti   universali:
  \begin{gather*}
    H_k(X;G) \cong H_k(X) \otimes G \oplus \tor{H_{n-1}(X), G}  \cong H_n(X) \otimes (\Z^n \oplus T) \oplus \tor{H_{n-1}(X), \Z^n \oplus T} \\
    \cong (\Z^{r_n} \oplus T') \otimes (\Z^{s_n} \oplus T) \oplus \tor{\Z^{t_n} \oplus T'', \Z^{s_n} \oplus T }
  \end{gather*}
  Ma essendo $ \Z^k $ libero per $ \Z^{t_n} \oplus T'' \cong T'' $ e $ \Z^{s_n} \oplus T \cong T $
  quindi:
  \[
    \tor{\Z^{t_n} \oplus T'', \Z^{s_n} \oplus T } = \tor{ T'', T }
  \]
  Inoltre per la bilinearità del prodotto tensore:
  \[
    \Z^{r_n} \otimes (\Z^{s_n} \oplus T) \cong \Z^{s_n} \otimes \Z^{r_n} \oplus \Z^{r_n} \otimes T
  \]
  Ma $ \Z^{r_n} \otimes \Z^{s_n} \cong \Z^{r_n s_n} $ e
  \[
    \Z^{r_n} \otimes T \cong (\Z \oplus \Z \dots ) \otimes T \cong (\Z \otimes T) \oplus (\Z \otimes T) \dots \cong (\Z \otimes T)^{r_n} \cong T^{r_n}
  \]
  Infine per lo stesso ragionamento: $ T' \otimes (\Z^{s_n} \oplus T) \cong T' \otimes T $
   Quindi:
   \[
     H_n(X; G) \cong \Z^{s_n r_n} \oplus T^{r_n} \oplus T' \oplus T \oplus \tor{T'', T}
   \]
   $ H_n(X;G) $ ha quindi una parte libera e delle parti di torsione che si
   calcolano sapendo fare $ \Z_h \otimes \Z_k $ (infatti $ T $ e $ T' $ sono fatte
   così).
 \end{osservation}
...
\[
  ...
  \end{tikzcd}
\]
\newmathsymb{cobord}{\delta}{Cobordo}
\newmathsymb{cohom}{H^p(X;G)}{Coomologia di $ X $ a valori in $ G $}
\newmathsymb{cohom}{S^p(X;G)}{Omomorfismi tra $ S_p(X) $ e $ G $}
...
\end{definition}
\newmathsymb{ext}{\ext{}}{Modulo di estensione}
\begin{lemma}
...
\begin{proof}
  ...
  \]
  Ma quindi $ v = 0 $ in quanto $ \tau_1 $ è iniettiva. Ma questo significa
  ...
  $ \sigma \circ \tau_2 \circ \tau_1 (v) = 0 $ in quanto
...
\begin{example}[Coomologia dello spazio proiettivo reale]
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Passando agli omomorfismi e considerando che $ \hom{2\Z, \Z_2} \cong \Z_2 $ :
  \[
    \begin{tikzcd}
      0 \rar & \Z_2 \rar & \Z_2 \rar & \Z_2 \rar & \ext{\Z_2, \Z_2} \rar & 0
    \end{tikzcd}
  \]
  La prima mappa è un omomorfismo iniettivo, ma tra $ \Z_2 $ e $ \Z_2 $ l'unico
  possibile è l'isomorfismo. La seconda mappa è la mappa nulla in quanto il suo
  nucleo è tutto $ \Z_2 $. La terza mappa è quindi iniettiva in quanto ha nucleo
  nullo, ma è anche suriettiva, quindi è un isomorfismo tra $ \ext{\Z_2, \Z_2} $
  e $ \Z_2 $.
...
\begin{align*}
  ...
  (\llbracket \phi \rrbracket, \llbracket \psi \rrbracket) & \mapsto \llbracket \phi \cup \psi \rrbracket
\end{align*}
...
\begin{proof}
  ...
  afferma che:
  \[
    H^k(X; \R) \cong \ext{H_k(X), \R} \oplus \hom{H_{k-1}(X), \R}
  \]
...
\begin{example}
  ...
  \begin{gather*}
    H^\star(\Pjc{2}) = \set{\tilde{a}_0 + \tilde{a}1 x + \tilde{a}2 x^2 | x^3 = 0} \\
    H^\star(\Pjc{2}) = \set{(\tilde{b}_0 + \tilde{b}_1 \alpha, \tilde{a}_0 + \tilde{a}_1 \beta) | \alpha^2 = 0, \; \beta^2 = 0}
  \end{gather*}
...
\begin{definition}
  ...
  dei vettori in $ x $.
  In generale una $ k $-forma si scrive:
  \[
    \omega_x = a_{1\dots k}(x) \d x^{1} \wedge \dots \wedge \d x^{k}
  \]
...
\begin{definition}
  ...
  Con:
  \[
    \d \omega = \sum_j \frac{\partial a_{1\dots k}(x )}{\partial x^j} \d x^j \wedge \d x^{1} \wedge \dots \wedge \d x^{k}
  \]
\end{definition}
\begin{osservation}
...
\end{example}
\newmathsymb{cohomderham}{H^p_{dR}(\M)}{Coomologia di de Rham di $ \M $}
\begin{definition}
...
\newmathsymb{cohomsupcomp}{H^p_{c}(\M)}{Coomologia a supporto compatto di $ \M $}
\begin{osservation}
...
\begin{lemma}
  ...
  compatto è covariante per aperti e non controvariante.
\end{lemma}
...
\begin{proof} \emph{Idee della dimostrazione.}
  ...
  \]
  con $ \phi \in \mathcal{C}^\infty $ a supporto compatto. Per verificarlo nel caso di
  $ \RN{} $ devo mostrare che in $ H^1_c(\RN{}) $ c'è una sola forma chiusa ma
  non esatta. In $ \RN{} $ tutte le $ 1 $-forme sono necessariamente chiuse in quanto
  non ci sono $ 2 $-forme. Considero la mappa lineare suriettiva:
  \begin{align*}
    \int_{\RN{n}} \colon \Omega^1(\RN{}) & \to \RN{} \\
    \omega & \mapsto \int_{\RN{n}} \omega
  \end{align*}
  Vale che:
  \[
    \ker{\int_{\RN{}}} = B^0(\RN{})
  \]
  Dove $ B^0(\RN{}) $ sono le forme esatte, infatti se
  $ \omega = \phi(t) \d t \in \ker{\int} $, è ben definito:
  \[
    \psi(x) = \int_{-\infty}^x \phi(t) \d t
  \]
  Ho che $ \psi $ è una $ 0 $-forma tale che
  $ \d \psi (x) = \psi'(x) \d x = \phi(x) \d x $, quindi $ \omega $ è esatta.
  Per il teorema fondamentale degli omomorfismi:
  \[
    \quot{\Omega^1(\RN{})}{\ker{\int_{\RN{}}}} \cong \RN{}
  \]
  Ma:
  \[
    \Omega^1(\RN{}) = Z^0(\RN{}) \text{ e } \ker{\int_{\RN{}}} = B^0(\RN{})
  \]
  Quindi:
  \[
    \quot{Z^0(\RN{})}{B^0{\RN{}}} \cong \RN{}
  \]
  Ma il membro di sinistra è proprio la definizione del gruppo di coomologia.
  Per $ n $ generico integro una alla volta tutte le variabili e ottengo il
  medesimo risultato.
  ...
  \subparagraph{Dimostrazione del punto quattro} Assumiamolo per vero.
  ...
  \[
    \begin{tikzcd}[nodes = {column sep = 6 pt, inner sep = 1.5pt}]
      ...
      (H_{dR}^{n-k+1}(V_1))^* \oplus (H_{dR}^{n-k+1}(V_2))^* \rar{\beta_1} &
      (H^{n-k+1}_{dR}(V_{12}))^* \rar{\beta_2} &(H^{n-k}_{dR}(\M))^* \rar{\beta_3} &
      (H_{dR}^{n-k}(V_1))^* \oplus (H_{dR}^{k}(V_2))^* \rar{\beta_4} & (H^{n-k}_{dR}(V_{12}))^*
    \end{tikzcd}
  \]
  ...
  $ \phi_2 = D_{V_{12}} $, $ \phi_3 = D $, $ \phi_5 = D_{V_1} \oplus D_{V_2} $ e $ \phi_5 = D_{V_{12}} $.
  \begin{osservation}
  ...
  \end{osservation}
  Bisogna dimostrare che i quadrati sono commutativi. Il penultimo
  considero $ [\alpha] \in H^k_{dR}(\M) $:
  \begin{gather*}
    [\alpha_1] = [\tau_1^\star(\alpha)] \in H^k_{dR}(V_1) \\
    [\alpha_2] = [\tau_2^\star(\alpha)] \in H^k_{dR}(V_2)
  \end{gather*}
  Cioè $ \alpha = \alpha_1 + \alpha_2 $, con $ \alpha_1 \in V_1 $ e $ \alpha_2 \in V_2 $ ma:
  \begin{gather*}
...
$ \rho \colon \Omega^p(X) \to \hom{S_p(X), \RN{}} $. Quindi definirò $ \rho $ tramite:
\[
  \begin{tikzcd}
...
\begin{proof} \emph{Idee della dimostrazione}.
  ...
  \item Se $ X $ è una varietà differenziale che ammette un ricoprimento finito
    di aperti de Rham tali che l'intersezione di suoi elementi sia de Rham
    allora è de Rham.
    \begin{definition}
      Si dice che $ X $ varietà differenziabile ha un \textbf{ricoprimento de
        Rham}\index{Ricoprimento de Rham} se ammette un ricoprimento $ \set{U_j} $
      tale che ogni insieme $ U_j $ è de Rham e ogni intersezione finita
      tra $ U_j $ è de Rham.
    \end{definition}
    Con questa definizione questo punto diventa equivalente a mostrare che ogni
    varietà differenziabile che ammette un ricoprimento finito de Rham è de
    Rham.
  \item Se la topologia di $ X $ ammette una base di aperti de Rham allora è de
    Rham.
    \begin{definition}
      Si dice che $ X $ varietà differenziabile ammette una \textbf{base} $ \mathcal{B} $
      \textbf{de Rham}\index{Base de Rham} se $ \mathcal{B} $ è un ricoprimento
      de Rham per $ X $ e $ \mathcal{B} $ è una base per la topologia di $ X $.
    \end{definition}
    Con questa definizione il punto diventa equivalente a mostrare che ogni
      varietà con una base de Rham è de Rham.
  ...
  Utilizzando le proprietà di additività dei gruppi di omologia
  e il fatto che per ipotesi $ H^p_{dR}(X_j) \cong H^p_\infty(X_j) $:
  \begin{gather*}
    H^p_{dR} = H^p_{dR}(\sqcup_j X_j) = \Pi H^p_{dR}(X_j) = \Pi H^p_\infty(X_j) \\
    \cong H^p_\infty(\sqcup_j, \RN{}) = H^p_\infty(X, \RN{})
  \end{gather*}
  Quindi $ H^p_\infty(X) \cong H^p_\infty(X, \RN{}) $.

  \subparagraph{Dimostrazione del punto due}
  Sia $ U $ aperto convesso in $ \RN{n} $.
  Si ricorda il lemma di Poincaré:
  \begin{theorem}[Lemma di Poincaré\index{Lemma di Poincaré}]
    Se $ U $ è un aperto convesso in $ \RN{n} $ allora
    $ H^p_{dR}(U) = 0 $ $ \forall p > 0 $.
  \end{theorem}
  Inoltre la coomologia è invariante per omotopia quindi
  $ H^p_\infty(U) \cong H^p_\infty(P) $ con $ P $ insieme formato da un solo punto e quindi
  per $ p > 0 $ i due gruppi sono certamente isomorfi. Bisogna verificarlo per
  $ p = 0 $, ma $ H^0_\infty(U) = \RN{} $ perché è la coomologia di un punto, e
  $ H^0_{dR}(U) = \RN{} $ in quanto $ U $ aperto convesso di $ \RN{} $.
  Bisogna controllare che l'omomorfismo $ \rho $ tra questi due spazi non sia nullo,
  in questo caso è automaticamente un isomorfismo.
  Ma:
  \begin{align*}
    \rho \colon H^0_{dR}(X) & \to H^p_\infty(X, \RN{}) \\
    f & \mapsto \int_\bullet f
  \end{align*}
  Scegliendo $ f = 1 $ si ottiene che $ \rho(f) $ non è nulla e quindi è
  isomorfismo.

  \subparagraph{Dimostrazione del punto tre}

  Siano $ \set{V_1, V_2} $ un ricoprimento di aperti de Rham, in cui si denota
  $ V_{12} = V_1 \cap V_2 $ per comodità. Per Mayer-Vietoris esiste la successione
  esatta:
  \[
    \begin{tikzcd}[nodes = {column sep = 6 pt, inner sep = 1.5pt}]
      H_{dR}^{k-1}(V_1) \oplus H_{dR}^{k-1}(V_2) \rar{} \dar{\rho_{V_1} \oplus \rho_{V_2}} &
      H^{k-1}_{dR}(V_{12}) \dar{\rho_{V_{12}}} \rar{} &
      H^k_{dR}(X) \rar{} \dar{\rho_{V_{12}}} & H_{dR}^{k}(V_1) \oplus H_{dR}^{k}(V_2) \rar{}  \dar{\rho_{V_1} \oplus \rho_{V_2}} & H^k_{dR}(V_{12}) \dar{\rho_{V_{12}}}  \\
      qH_{dR}^{k-1}(V_1) \oplus H_{dR}^{k-1}(V_2) \rar{} & H^{k-1}_{dR}(V_{12})
      \rar{} & H^k_{dR}(X) \rar{} & H_{dR}^{k}(V_1) \oplus H_{dR}^{k}(V_2)
      \rar{} & H^k_{dR}(V_{12})
    \end{tikzcd}
  \]
  Siccome per i punti precedenti tutti i $ \rho $ sono isomorfismi eccetto quello
  centrale, se dimostro che i quadrati sono commutativi per il lemma dei cinque
  ho dimostrato il punto. Con una certa fatica si dimostra che in effetti
  i quadrati commutano.

  Nel caso il ricoprimento sia $ \set{U_1, U_2, \dots, U_k} $ si definisce
  $ V_1 = U_1 \cup \dots U_{k-1} $ e $ V_2 = U_k $. Siccome sia $ V_1 $ che
  $ V_2 $ sono de Rham (in quanto unione di de Rham è de Rham) si può applicare
  il ragionamento precedente se mostro che $ V_1 \cap V_2 $ è de Rham.
  Ma:
  \[
    V_1 \cap V_2 = (U_1 \cup \dots U_{k-1}) \cap U_k = (U_1 \cap U_k) \cup \dots (U_{k-1} \cap U_k)
  \]
  Siccome $ U_i \cap U_k $ sono de Rham e unione di de Rham è de Rham $ V_1 \cap V_2 $
  è de Rham.

  \subparagraph{Dimostrazione del punto quattro}

  \begin{definition}
    Sia $ X $ è una varietà differenziabile, una funzione continua $ f \colon X \to \RN{} $
    si dice \textbf{esaustione in compatti}\index{Esaustione in compatti} di $ X $
    se $ f^{-1}((-\infty, c]) $ è compatto $ \forall c \in \RN{} $.
  \end{definition}

  Si dimostra che ogni varietà differenziabile ammette un'esaustione in compatti
  positiva e liscia. Considero questa esaustione $ f $ per $ X $ varietà
  differenziabile, si definiscono gli insiemi compatti
  $ \tilde{A}_n = f^{-1}((-\infty, n]) $ e gli insiemi
  \[
    A_m = \set{ x \in X | m \leq f(x) \leq m + 1}
  \]
  Gli $ A_m $ sono chiusi in $ \tilde{A}_{m+1} $ che sono compatti quindi sono compatti.
  Definendo:
  \[
    A'_m = \set{x \in X | m - \frac{1}{2} < f(x) < m + \frac{3}{2} }
  \]
  Questi sono aperti. Siccome esiste una base de Rham $ \forall x \in A'_m $ esiste un elemento
  della base $ U^x_m $ che contiene $ x $ e sta in $ A'_m $ (per la definizione di base).

  L'insieme $ \set{U_m^x}_{x \in A_m} $ è quindi ben definito e si ha che
  $ A_m \subseteq \bigcup_{x\in A_m} U^x_m $, ma $ A_m $ è compatto quindi per definizione il
  ricoprimento è finito. Sia $ B_m = \bigcup^n_i U^i_m $, essendo
  $ \set{U_m^i} $ ricoprimento finito de Rham di $ B_m $ allora $ B_m $ è de
  Rham. Ho quindi un'infinità numerabile di $ B_m $ de Rham.
  Siano:
  \[
    \mathcal{U} = \bigcup_{m = 1\;\mathrm{mod} (2)} B_m \quad  \mathcal{V} = \bigcup_{m = 0\; \mathrm{mod} (2)} B_m
  \]
  Questi insiemi sono tali che $ \mathcal{U} \cup \mathcal{V} = X $ in quanto per
  ipotesi prendo tutti gli $ A_n $ che coprono $ X $. Verificando che
  $ \mathcal{U} $ e $ \mathcal{V} $ e $ \mathcal{U} \cap \mathcal{V} $ sono de Rham
  si dimostra il punto.

  \subparagraph{Dimostrazione del punto cinque} Sia $ U $ aperto di $ \RN{n} $,
  può essere ricoperto da palle aperte, infatti una base $ \mathcal{B} $ della
  topologica $ \RN{n} $ è fornita dalle palle aperte $ B_r(X) $. Per il punto
  precedente se mostro che $ \mathcal{B} $ è una base de Rham allora $ U $ è
  de Rham. L'unica cosa che bisogna mostrare è che l'intersezione di elementi
  di $ \mathcal{B} $ sono de Rham, ma l'intersezione di palle è un insieme convesso
  quindi è de Rham.
  ...
  cinque $ V_\alpha $ sono de Rham, quindi per il lemma precedente $ U_\alpha $ sono de
  Rham. Ora basta mostrare che $ X $ ammette una base di aperti costruiti a
  partire da questi $ U_\alpha $ in questo modo per il punto quattro $ X $ è de Rham.
  Per definizione di atlante le carte forniscono un ricoprimento de Rham $ X $,
  infatti l'intersezione finita di $ U_\alpha $ è un aperto de Rham in quanto è
  mappato in un aperto di $ \RN{n} $ dalla restrizione delle carte
  sull'intersezione.
\end{proof}
